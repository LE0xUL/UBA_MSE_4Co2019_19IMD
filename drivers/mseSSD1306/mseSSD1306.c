...
static int mseSSD1306_open(struct inode *inodep, struct file *filep){
    // numberOpens++;
    // printk(KERN_INFO "mseSSD1306: Device has been opened %d time(s)\n", numberOpens);
    if( 0 == numberOpens++ )
    {
        clearDisplaySSD1306();
        homeDisplaySSD1306();
...
static ssize_t mseSSD1306_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
    ...
    // printk(KERN_INFO "mseSSD1306: Received %s : %zu characters from the user\n", message , len);
    sentStringSSD1306( message , len );
    // msleep(1000);
    // printk(KERN_INFO "mseSSD1306: WRITE\n");
    return len;
}
...
//    printk(KERN_INFO "mseSSD1306: Device successfully closed\n");
//    sentImgSSD1306( tuxV );
   return 0;
}
...
static int mseSSD1306_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (IS_ERR(mseSSD1306Device)){               // Clean up if there is an error
        class_destroy(mseSSD1306Class);           // Repeated code but the alternative is goto statements
        unregister_chrdev(majorNumber, DEVICE_NAME);
